...
if [ "$BACKEND" = "claude" ]; then
  ...
  # Default Ollama model if not explicitly set
  if [ -z "$MODEL" ] || [ "$MODEL" = "claude-opus-4-5-20251101" ]; then
    MODEL="qwen3.5:0.8b"
  fi
else
...
non_streaming_api_call() {
  if [ "$BACKEND" = "claude" ]; then
    ...
    # Ollama non-streaming call (native Ollama /api/chat)
    # Build messages array from history + prompt, including system prompt
    ollama_messages=$(echo "$messages" | jq --arg system "$SYSTEM_PROMPT" '[{role:"system", content:$system}] + ([.[] | {role, content}])')

    json_payload=$(jq -n \
      --arg model "$MODEL" \
      --argjson messages "$ollama_messages" \
      '{model: $model, messages: $messages, think: false, stream: false}')

    # Optional debug: show outgoing Ollama payload when DEBUG_OLLAMA is set (single-line JSON)
    if [ -n "${DEBUG_OLLAMA:-}" ]; then
      printf 'OLLAMA_REQUEST_JSON:%s\n' "$json_payload" >&2
    fi
    ...
    if [ "$http_code" != "200" ]; then
      err=$(echo "$response_body" | jq -r '.error.message // .error // .' 2>/dev/null || echo "$response_body")
      echo "API Error ($http_code): $err"
      rm -f "$error_log"
      return 1
    fi

    assistant_response=$(echo "$response_body" | jq -r '.message.content // empty')
    rm -f "$error_log"

    if [ -z "$assistant_response" ]; then
      echo "API Error ($http_code): Empty response"
      return 1
    fi
...
if [ "$BACKEND" = "claude" ]; then
  ...
  # Ollama backend: non-streaming only for now
  assistant_response=$(non_streaming_api_call)
  if [ $? -ne 0 ]; then
    exit 1
  fi
